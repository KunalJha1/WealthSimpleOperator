...
type NoteTab = "notes" | "transcript" | "ai_summary";

type QuickActionKind = "create_task" | "schedule_meeting" | "send_email";
...
function buildQuickActionDraft(kind: QuickActionKind, alert: AlertDetail): QuickActionDraft {
  const firstName = alert.client.name.split(" ")[0] ?? alert.client.name;
  ...
  if (kind === "send_email") {
    return {
      ...
      statusLine: "A proposed email which can be sent after advisor approval.",
      subject: `Follow-up on your portfolio plan - ${displayDate}`,
      body: `Hi ${firstName},

I hope you are doing well. I wanted to follow up after reviewing your portfolio and recent goals. I would like to discuss your current risk allocation, recent portfolio movement, and the next steps we can take together.

If you are available, I can schedule a short meeting and share a clear action plan afterward.

Kind regards,
Advisor Team`,
      footer: `Wealthsimple Private Wealth
...
Confidentiality Notice: This message may contain confidential information and is intended only for the named recipient.`
  ...
  if (kind === "schedule_meeting") {
    return {
      kind,
      title: "Proposed Email: Meeting Request",
      statusLine: "A proposed email which can be sent after advisor approval.",
      subject: `Meeting proposal for ${alert.client.name}`,
      body: `Hi ${alert.client.name},

I wanted to see if you are available for a meeting on one of the following times:
- Tuesday 10:00 AM
- Wednesday 2:00 PM
- Friday 11:30 AM

I would like to discuss your portfolio, your current risk allocation, and the next steps we can take to keep your plan aligned with your goals.
...
Confidentiality Notice: This message may contain confidential information and is intended only for the named recipient.`
  ...
  return {
    kind,
    title: "Agent Draft: Advisor Task",
    statusLine: "Internal task generated by AI agent. Human approval required before queueing.",
    subject: `Create follow-up task for ${alert.client.name}`,
    body: `Create a high-priority advisor task:
- Validate latest risk movement against client goals
- Prepare client-ready recommendation summary
- Set follow-up completion target for next business day
- Log final action in audit trail`
  };
}
...
function buildFallbackClientProfileView(alert: AlertDetail): ClientProfileView {
  ...
  return {
    ...
    recent_meeting_notes: [
      {
        id: 1001,
        title: "Portfolio Volatility Check-in Call",
        date: formatDateISO(lastMeeting),
        note:
          "Client asked for a clear breakdown of short-term volatility impact and available de-risking options before next quarter.",
        action_required: ["Send updated risk breakdown", "Book follow-up review"],
        meeting_type: "phone_call",
        call_transcript:
          "Advisor: Thanks for taking the call. I wanted to walk through the recent movement in your portfolio.\nClient: I noticed the swings this month and want to understand if I should reduce equity exposure now.\nAdvisor: We can review what changed and map options against your home purchase timeline.\nClient: That sounds good. I want a plan that keeps me on track but reduces stress.\nAdvisor: Agreed. I will send a short recommendation summary and a meeting invite for final decisions.",
        ai_summary:
          "Client is concerned about near-term portfolio swings and asked for a lower-volatility path aligned to the home purchase timeline. Advisor committed to sending a concise recommendation and follow-up meeting options.",
        ai_action_items: [
          "Prepare recommended allocation scenarios",
          "Share written summary with timeline impact",
          "Schedule advisor review meeting"
        ],
        has_transcript: true
      },
      {
        ...
        date: formatDateLabel(lastMeeting, -18),
        note:
          "Reviewed annual financial planning milestones and confirmed retirement contribution pacing remains on track.",
        action_required: [],
        meeting_type: "meeting"
      },
      {
        id: 1003,
        title: "Income Planning Phone Call",
        date: formatDateLabel(lastMeeting, -43),
        note:
          "Discussed withdrawal sequencing and tax treatment for expected home down payment in late 2026.",
        action_required: ["Tax planning"],
        meeting_type: "phone_call",
        call_transcript:
          "Advisor: I wanted to review how we can structure withdrawals for the home purchase.\nClient: My main concern is avoiding unnecessary tax impact.\nAdvisor: We can sequence from cash and fixed income first, then revisit equities once market conditions improve.\nClient: Perfect, please include that in the next written update.",
        has_transcript: true
      }
    ],
    financial_goals: [
...
export function ClientDetailsPanel({ alert }: { alert: AlertDetail }) {
  ...
  const [activeNoteTab, setActiveNoteTab] = useState<Record<number, NoteTab>>({});
  ...
  function handleApproveDraft() {
    ...
    notifyUser(`${quickActionDraft.title} approved and queued for execution.`);
  }

  function handleRequestChanges() {
    if (!quickActionDraft) return;
    setApprovalStatus("changes_requested");
    notifyUser("Draft marked for edits. Update and approve when ready.");
  }
    ...
    <div className="rounded-xl border border-ws-border bg-white p-4 space-y-4">
      ...
      <div className="space-y-4">
        ...
        {showNoteForm && (
          <div className="rounded-lg border-2 border-blue-200 bg-blue-50 p-3 space-y-2">
            ...
            <textarea
              ...
              className="w-full rounded border border-blue-300 bg-white px-2 py-1.5 text-xs outline-none focus:border-blue-500 min-h-40"
            />
            <div className="flex gap-2 pt-1">
              <button
                type="button"
        ...
        <div className="rounded-xl border border-blue-200 bg-blue-50/40 p-4 space-y-3">
          <div className="text-xs font-bold uppercase tracking-wide text-blue-700">Recent Call Transcripts</div>
                  ...
                  <>
                    ...
                    <div className="flex gap-1 border-b border-ws-border">
                      {["notes", "transcript", "ai_summary"].map((tab) => (
                        <button
                          key={tab}
                          onClick={() => setActiveNoteTab(prev => ({ ...prev, [noteId]: tab as NoteTab }))}
                    ...
                    {activeTab === "ai_summary" && (
                      <div className="space-y-2">
                        {!isMostRecentTranscript ? (
                          <div className="text-xs text-ws-muted">
                            AI summary is only available for the most recent call transcript.
                          </div>
                        ) : note.ai_summary ? (
                          <>
                            ...
                            {note.ai_action_items && note.ai_action_items.length > 0 && (
                              <div className="space-y-1">
                                ...
                                <ul className="list-disc list-inside space-y-1">
                                  {note.ai_action_items.map((item, idx) => (
                                    <li key={`${noteId}-ai-${idx}`} className="text-xs text-gray-700">{item}</li>
                                  ))}
                                </ul>
                              </div>
                            )}
        ...
        {quickActionDraft && (
          <div className="mt-3 rounded-xl border border-amber-200 bg-amber-50 p-4 space-y-3">
            <div className="space-y-1">
              ...
              <div className="text-xs text-amber-900">{quickActionDraft.statusLine}</div>
            ...
            <div className="space-y-1">
              ...
              <input
                type="text"
                value={quickActionDraft.subject}
                onChange={(e) => handleDraftChange("subject", e.target.value)}
                className="w-full rounded border border-amber-300 bg-white px-2 py-1.5 text-xs text-gray-900 outline-none focus:border-amber-500"
              />
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Button
                type="button"
                variant="secondary"
                className="text-xs px-3 py-1.5 bg-white border-amber-300 text-amber-800 hover:bg-amber-100"
